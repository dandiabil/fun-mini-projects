...
const sentences =
  "Lorem ipsum dolor sit amet consectetur, adipisicing elit. Beatae dolorem delectus rerum veritatis amet eveniet ipsum necessitatibus voluptatibus est eligendi";
...
const start = () => {
  ...
  sentencesArray.map((word, i) => {
    let display = "";
    if (i === 0)
      sentencesDisplay.innerHTML += `<span class="word selected-word">${word}</span> `;
    else {
...
inputWord.addEventListener("keyup", (e) => {
  ...
  if (e.key === " ") {
    if (e.target.value.trim() === sentencesArray[i]) {
